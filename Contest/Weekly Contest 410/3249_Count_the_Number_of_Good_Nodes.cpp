...
public:
    ...
    int helper(vector<vector<int>>& edge, int parent, int node){
        int count = 1;
        int sub_count = -1;
        bool balance = true;
        for (int child : edge[node]){
            if (child == parent) continue;
            int temp = helper(edge, node, child);
...
//             e[to].push_back(from);
//         }
//         // Just need to add parent in dfs(helper) function, no need to use visited to create the whole graph
