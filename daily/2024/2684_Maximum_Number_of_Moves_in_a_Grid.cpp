#include <iostream>
#include <vector>

using namespace std;

class Solution {
public:
    int maxMoves(vector<vector<int>>& grid) {
        int m = grid.size();
        int n = grid[0].size();
        vector<int> dp(m, 1);
        for (int j = 1; j < n; j++){
            vector<int> dp_new(m, 0);
            bool update = false;
            for (int i = 0; i < m; i++){
                if (i - 1 >= 0 && dp[i-1] && grid[i][j] > grid[i-1][j-1]) dp_new[i] = 1;
                dp_new[i] |= (dp[i] && grid[i][j] > grid[i][j-1]);
                if (i + 1 < m && dp[i+1] && grid[i][j] > grid[i+1][j-1]) dp_new[i] = 1;
                if (dp_new[i]) update = true;
            }
            swap(dp, dp_new);
            if (!update) return j-1;
        }
        return n-1;
    }
};

int main(){
    vector<vector<int>> grid = {{2,4,3,5},{5,4,9,3},{3,4,2,11},{10,9,13,15}};
    // vector<vector<int>> grid = {{3,2,4},{2,1,9},{1,1,7}};
    // vector<vector<int>> grid = {{187,167,209,251,152,236,263,128,135},{267,249,251,285,73,204,70,207,74},{189,159,235,66,84,89,153,111,189},{120,81,210,7,2,231,92,128,218},{193,131,244,293,284,175,226,205,245}};
    // vector<vector<int>> grid = {{57,213,269,42,135,1,130,16,5,283,181,46,56,30},{170,260,222,279,276,50,182,264,256,255,233,82,300,19},{300,68,261,241,96,205,1,254,201,95,124,132,40,220},{212,38,50,38,176,271,109,27,83,195,165,7,180,224},{238,106,44,264,157,180,94,92,119,142,59,269,275,189},{35,34,226,241,45,195,200,178,104,92,255,170,49,91},{94,137,145,78,49,243,263,113,230,144,249,192,36,276},{271,241,12,126,232,192,294,212,239,199,49,106,177,104},{243,206,191,14,110,62,137,184,185,12,259,217,285,209},{124,197,299,299,4,36,245,160,182,300,7,170,50,67},{77,39,189,185,277,223,122,181,55,97,97,85,215,287},{124,42,198,38,204,87,172,9,102,232,112,71,186,296},{13,101,254,158,289,112,89,172,194,269,27,171,149,223}};
    Solution S;
    cout << S.maxMoves(grid) << endl;
    return 0;
}